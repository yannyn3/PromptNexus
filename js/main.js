...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.app = {
    ...
    initElements() {
      ...
      this.totalCategoriesCount = document.getElementById(
      ...
      // 安全设置
      this.requirePasswordCheckbox =
        document.getElementById("require-password");
      this.adminPasswordSetting = document.getElementById(
    ...
    initEventListeners() {
      ...
      this.promptsTab.addEventListener("click", () =>
      ...
      document
        ...
        .addEventListener("click", () => this.openNewPromptForm());
      document
        .getElementById("add-first-prompt-btn")
        ?.addEventListener("click", () => this.openNewPromptForm());

      // 顶部添加提示词按钮
      document
        .getElementById("header-add-prompt-btn")
      ...
      document
        ...
        .addEventListener("click", () => this.closePromptModal());
      ...
      // GitHub同步
      document
        .getElementById("save-github-settings")
        ?.addEventListener("click", () => {
      ...
      document
        ...
        .addEventListener("click", () => this.closePasswordModal());
      ...
      document
        ...
        .addEventListener("click", () => this.saveSecuritySettings());
    ...
    changeView(view) {
      ...
      // 更新标签状态
      [this.dashboardTab, this.promptsTab, this.settingsTab].forEach((tab) =>
        tab.classList.remove("active"),
      );
      [this.dashboardView, this.promptsView, this.settingsView].forEach(
    ...
    savePrompt: async function () {
      ...
      const note = this.promptNoteInput
        ? this.promptNoteInput.value.trim()
        : "";
          ...
          categories.forEach((category) => {
            ...
            if (categoriesList) {
              ...
              categoryItem.className =
                "flex justify-between items-center glass glass-light dark:glass-dark rounded-xl px-4 py-2";
              ...
              // 添加删除事件
              categoryItem
                .querySelector(".category-delete-btn")
                .addEventListener("click", () => {
    ...
    renderPrompts(prompts) {
      ...
      prompts.forEach((prompt, index) => {
        ...
        promptEl.className =
          "prompt-item glass-card p-5 cursor-pointer prompt-card hover-3d";
        // 添加交错的悬浮动画
        if (index % 3 === 0) {
        ...
        if (prompt.category) {
          const categoryOption = this.promptCategoryInput.querySelector(
            `option[value="${prompt.category}"]`,
          );
        ...
        promptEl
          .querySelector(".prompt-delete-btn")
          .addEventListener("click", (e) => {
            e.stopPropagation();
      ...
      if (prompts.length === 0) {
        ...
        noPromptsEl
          .querySelector("#add-first-prompt-btn")
          .addEventListener("click", () => {
            this.openNewPromptForm();
          });
    ...
    toggleView() {
      ...
      if (isListView) {
        ...
        this.promptsContainer.classList.add(
          "grid-cols-1",
          "md:grid-cols-2",
          "lg:grid-cols-3",
        );
        ...
        this.promptsContainer.classList.remove(
          "grid-cols-1",
          "md:grid-cols-2",
          "lg:grid-cols-3",
        );
      ...
      prompts.forEach((prompt) => {
        if (prompt.category) {
          categoryCounts[prompt.category] =
            (categoryCounts[prompt.category] || 0) + 1;
        } else {
          uncategorizedCount++;
        }
      });
    ...
    loadGitHubSettings: function () {
      ...
      const autoLoadCheckbox = document.getElementById("github-auto-load");
      ...
      if (githubTokenInput && gistIdInput) {
        ...
        if (autoLoadCheckbox) {
          autoLoadCheckbox.checked = settings.autoLoad !== false; // 默认为true
        }
      }
    },
    ...
    saveGitHubSettings: function () {
      ...
      if (githubTokenInput && gistIdInput) {
        ...
        const autoLoad = autoLoadCheckbox ? autoLoadCheckbox.checked : true;

        const success = this.storage.saveGitHubSettings(
          token,
          gistId,
          autoLoad,
        );
      ...
      try {
        ...
        if (!silent) {
          ...
          const stickyNotification =
            document.querySelector(".notification-info");
          if (
            stickyNotification &&
            stickyNotification.querySelector(".notification-title")
              .textContent === "正在同步"
          ) {
          ...
          if (result.success) {
            ...
            window.showNotification(
              "error",
              "同步失败",
              result.message || "与GitHub Gist同步数据时发生错误。",
            );
          }
        }
        ...
        if (
          result.gistId &&
          result.gistId !== this.storage.getGitHubSettings().gistId
        ) {
        ...
        if (!silent) {
          ...
          window.showNotification(
            "error",
            "同步异常",
            `与GitHub Gist同步时发生严重错误: ${error.message}`,
          );
        }
    ...
    loadSecuritySettings() {
      ...
      this.requirePasswordCheckbox.checked =
        settings.requirePasswordForAdd === true;
    },
